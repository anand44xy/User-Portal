...
UserRouter.post('/register', async (req, res) => {
    try {
        ...
        if (userCount == 1) {
            res.send({ msg: 'User with this email already exists!', statusType: 0 });
            return;
        }

        // Create and save the new user if the email is not registered
        ...
        user.save()
            .then(() => {
                res.send({ msg: "User registered successfully!", statusType: 1 });
            })
            .catch((error) => {
                res.send({ msg: "Unable to register the user!", statusType: 0 });
            });
    } catch (error) {
